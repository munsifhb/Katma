...
export default function Shops() {
  const { shops, filterShops, isOpen, setIsOpen } = useGeneral();
  return (
    <div onClick={() => setIsOpen(false)} className='bg-gray-200  min-h-screen pt-20 pb-20'>
      ...
      <div className='mt-16 w-4/5 md:w-3/5 sm:w-5/6 mx-auto mb-16'>
        <div className='grid grid-cols-4 md:grid-cols-3 sm:grid-cols-2 gap-6'>
          ...
          <Link to={`/shop/${shop.id}`} key={shop.id} onClick={() => setIsOpen(false)} className="bg-white rounded-lg shadow-md p-2 overflow-hidden flex flex-col items-center hover:shadow-xl transition-shadow duration-300 ease-in-out">
